...
router.get('/settings', async (req, res, next) => {
  ...
  const dbResponse = await dbConnection.query(
    `SELECT firstname, lastname, nickname FROM users WHERE user_id = '${userId}' AND active = true LIMIT 1;`,
  );
...
router.put(
  ...
  async (req, res, next) => {
    ...
    await dbConnection.query(
      `UPDATE users SET nickname = '${nickname}', firstname = '${firstName}', lastname = '${lastName}', active = true WHERE user_id = '${userId}' AND active = true;`,
    );
...
router.put(
  ...
  async (req, res, next) => {
    ...
    const dbResponse = await dbConnection.query(
      `SELECT password FROM users WHERE user_id = '${userId}' AND active = true LIMIT 1;`,
    );
        ...
        bcrypt.hash(newPassword, 10, async (errorHash, hash) => {
          if (!errorHash) {
            const a = await dbConnection.query(
              `UPDATE users SET password = '${hash}' WHERE user_id = '${userId}' AND active = true;`,
            );
